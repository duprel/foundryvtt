...
let folder01 = "modules/jb2a_patreon/Library/Generic/Explosion/";
//anFile is the name of the file used for the animation
let anFile = `${folder01}Explosion_02_Orange_400x400.webm`;
...
async function Cast() {
    var myStringArray = Array.from(canvas.tokens.controlled)[0];
    var arrayLength = canvas.tokens.controlled.length;
    for (var i = 0; i < arrayLength; i++) {

        let mainTarget = myStringArray;
        let tarScale = ((myStringArray.data.width + myStringArray.data.height) / 5);
