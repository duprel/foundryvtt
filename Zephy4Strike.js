...
if (s_actor?.data.items.find(i => i.name === "Zephyr Strike") === undefined){
...
if (hasAvailableSlot(s_actor)) {
    ...
    new Dialog({
        title: "Zephyr Strike: Usage Configuration",
        content: `
        <form id="smite-use-form">
            <p>` + game.i18n.format("DND5E.AbilityUseHint", {name: "Zephyr Strike", type: "spell"}) + `</p>
...
function smite(actor, slotLevel, criticalHit, consume) {
    ...
    targets.forEach(target => {
        ...
        const flavor = `Macro Zephyr Strike - ${game.i18n.localize("DND5E.DamageRoll")} (${game.i18n.localize("DND5E.DamageForce")})`;
        let damageRoll = new Roll(`${numDice}d8[force]`).roll();
        new MidiQOL.DamageOnlyWorkflow(actor, token, damageRoll.total, "force", [target], damageRoll, {flavor: "Zephyr Strike - Damage Roll (Force)"})
    })
