...
(() => {
    if (args[0] === "on") {
        ...
        if (s_actor?.data.items.find(i => i.name === "Absorb Elements") === undefined) {
        ...
        if (hasAvailableSlot(s_actor)) {
            ...
            new Dialog({
                title: "Absorb Elements: Usage Configuration",
                content: `
        <form id="smite-use-form">
            <p>` + game.i18n.format("DND5E.AbilityUseHint", { name: "Absorb Elements", type: "spell" }) + `</p>
        ...
        function smite(actor, slotLevel, criticalHit, consume) {
            ...
            targets.forEach(target => {
                ...
                const flavor = `Macro Absorb Elements - ${game.i18n.localize("DND5E.DamageRoll")} (${game.i18n.localize("DND5E.DamageForce")})`;
                let damageRoll = new Roll(`${numDice}d6[force]`).roll();
                new MidiQOL.DamageOnlyWorkflow(actor, token, damageRoll.total, "force", [target], damageRoll, { flavor: "Absorb Elements - Damage Roll (Force)" })
            })
