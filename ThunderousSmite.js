...
(() => {
    if (args[0] === "on") {
        ...
        let maxSpellSlot = 2; //  Highest spell-slot level that may be used.
        ...
        if (s_actor?.data.items.find(i => i.name === "Wrathful Smite") === undefined) {
        ...
        if (hasAvailableSlot(s_actor)) {
            ...
            new Dialog({
                title: "Wrathful Smite: Usage Configuration",
                content: `
        <form id="smite-use-form">
            <p>` + game.i18n.format("DND5E.AbilityUseHint", { name: "Wrathful Smite", type: "spell" }) + `</p>
        ...
        function smite(actor, slotLevel, criticalHit, consume) {
            ...
            targets.forEach(target => {
                ...
                let damageRoll = new Roll(`${numDice}d6[psychic]`).roll();
                new MidiQOL.DamageOnlyWorkflow(actor, token, damageRoll.total, "psychic", [target], damageRoll, { flavor: "Wrathful Smite - Damage Roll (Psychic)" })
            })
        ...
        console.log("***************************", spellDC);
        if (saveT < spellDC) {
            game.cub.addCondition("Frightened", [...game.user.targets], { allowDuplicates: false, replaceExisting: false });
        }
    }
