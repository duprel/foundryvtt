...
(() => {
    if (args[0] === "on") {
        ...
        if (s_actor?.data.items.find(i => i.name === "Ensnaring Strike") === undefined) {
        ...
        if (hasAvailableSlot(s_actor)) {
            ...
            new Dialog({
                title: "Ensnaring Strike: Usage Configuration",
                content: `
        <form id="smite-use-form">
            <p>` + game.i18n.format("DND5E.AbilityUseHint", { name: "Ensnaring Strike", type: "spell" }) + `</p>
        ...
        function smite(actor, slotLevel, criticalHit, consume) {
            ...
            targets.forEach(target => {
                ...
                new MidiQOL.DamageOnlyWorkflow(actor, token, damageRoll.total, "piercing", [target], damageRoll, { flavor: "Ensnaring Strike - Damage Roll (Piercing)" })
            })
        ...
        let conSave = target.actor.data.data.abilities.con.save;
        let saveRoll = new Roll("d20 + @conS", { conS: conSave }).roll()
        saveRoll.toMessage({ flavor: "Constitution Saving Throw" });
        var saveT = saveRoll.total + conSave;
        console.log("***************************", saveT);
        console.log("***************************", spellDC);
        if (saveT < spellDC) {
            game.cub.addCondition("Restrained", [...game.user.targets], { allowDuplicates: false, replaceExisting: false });
            if (!actor) {
                ...
                var recipients = [];
                let alertData = { label: "" },
                    dialogContentlabel = `<div><span style="flex:1">Alert name: <input name="label" style="width:350px"value="Ensnaring Strike DoT"/></span></div>`, //text box for inputting alert name
                    ...
                    dialogContentmessage = `<div><span style="flex:1">Message: <input name="message" style="width:350px" value="Roll a STR save DC: ${spellDC}"/></span></div>`, //text box for inputting end message
                    ...
                    d = new Dialog({                                                            //*
                        ...
                        buttons: {                                                                  //*
                                ...
                                callback: (html) => {
                                    ...
                                    alertData.macro = "Ensnaring Strike DoT Roll";   //sets the macro to be triggered on end, null for none
